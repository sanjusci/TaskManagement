...
def define_usage(params=None, returns=None):
    def decorator(function):
        ...
        methods = [method.upper() for method in cls.http_method_names if
                   method != 'options']
...
class TokenAuthentication(BaseAuthentication):
    ...
    * user -- The user to which the token belongs
    ...
    def authenticate_credentials(self, key):
        ...
        token_time = token.created
        from django.utils import timezone
        now = timezone.now()
        diff = (now - token_time).seconds / 60
        if diff > 21600:
            token.delete()
            raise exceptions.AuthenticationFailed(_('Token Expires. Please login again'))
        else:
